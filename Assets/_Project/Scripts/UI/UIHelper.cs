...
public static class UIHelper
{
    ...
    public static Text CrearTexto(GameObject p, string n, string t, int sz,
        FontStyle est, Color c, TextAnchor al, Vector2 am, Vector2 aM, Vector2 pv, Vector2 pos, Vector2 tam)
    {
        return CrearTexto(p != null ? p.transform : null, n, t, sz, est, c, al, am, aM, pv, pos, tam);
    }

    public static Text CrearTexto(GameObject p, string n, string t, int sz, FontStyle est, Color c,
        TextAnchor al, Vector2 am, Vector2 aM)
    {
        return CrearTexto(p != null ? p.transform : null, n, t, sz, est, c, al, am, aM, Vector2.one * 0.5f, Vector2.zero, Vector2.zero);
    }

    public static Text CrearTexto(GameObject p, string n, string t, int sz, FontStyle est, Color c,
        TextAnchor al, Vector2 am, Vector2 aM, Vector2 pos, Vector2 tam)
    {
        return CrearTexto(p != null ? p.transform : null, n, t, sz, est, c, al, am, aM, Vector2.one * 0.5f, pos, tam);
    }

    public static Text CrearTexto(Transform p, string n, string t, int sz, FontStyle est, Color c,
        TextAnchor al, Vector2 am, Vector2 aM)
    {
        return CrearTexto(p, n, t, sz, est, c, al, am, aM, Vector2.one * 0.5f, Vector2.zero, Vector2.zero);
    }

    public static Text CrearTexto(Transform p, string n, string t, int sz, FontStyle est, Color c,
        TextAnchor al, Vector2 am, Vector2 aM, Vector2 pos, Vector2 tam)
    {
        return CrearTexto(p, n, t, sz, est, c, al, am, aM, Vector2.one * 0.5f, pos, tam);
    }
        ...
        Color cF, Color cB, UnityEngine.Events.UnityAction acc)
    {
        ...
        rt.anchoredPosition = pos;
        rt.sizeDelta = tam;
        Outline ob = btn.AddComponent<Outline>();
        ob.effectColor = cB;
        ob.effectDistance = new Vector2(1f, 1f);
